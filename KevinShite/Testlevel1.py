...
removedLife = True
def draw(canvas):
    global interactions, addedInteractions, numberOfPlatforms, framecount, timer,numberOfGoats,goats,numberOfChikens, chickens, removedLife,scoreCount
    ...
    for i in range(len(platforms.getCoords())):
        ...
        for j in range(len(goats)):
            ...
            if goats[j].pos.y > 630:
                goatsToPop.append(goats[j])
                numberOfGoats-=1
            # for k in range(len(goatsToPop)):
            #     goats.remove(goatsToPop[k])
        for j in range(len(goatsToPop)):
            goats.remove(goatsToPop[j])

    framecount += 1
    if random.randint(0,1) > 0.5 and framecount % 60 == 0 and numberOfChikens <3:
        makeChicken()
        numberOfChikens+=1
    ...
    for j in range(len(chickens)):
        chickens[j].update(canvas)

    for j in range(len(chickens)):
        if chickens[j].isColliding(player1):
            if chickens[j].hit(player1):
                player1.scoreCount+=150
                chickensToPop.append(chickens[j])
                numberOfChikens-=1
            else:
                player1.lifePoints-=1

    for j in range (len(chickensToPop)):
        chickens.remove(chickensToPop[j])
...
def makeChicken():
    global chickens
    chicken = ChickenMonster2(2, player1)
